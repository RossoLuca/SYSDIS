...
-export([start_link/0, init/1, drone_synchronizer/8, send_update_table/3, agreement_loop/7, go_to_agreement/2]).
...
init(Id) ->
    ...
    DroneHubProcess = {list_to_atom(os:getenv("DRONE_HUB_PROCESS")), list_to_atom(os:getenv("DRONE_HUB_HOST"))},
    ...
    case http_utils:createConnection(maps:get(rest_endpoint, Configuration)) of
        ...
        Connection ->
            ...
            if Size > 0 ->
                ...
                sync_loop(Id, Configuration, DroneState, CollisionTable, FilteredSynchronizationMap, NewDrones, PersonalCollisions, ToNotUpdate, Synchronizers);
            true ->
                CollisionTable = #{
...
receive_configuration() ->
    receive
        {config, CodedPid, RestEndpoint, Velocity, Drone_size, Notify_Threshold, Recovery, Height, Start, Current, End, State, Fallen} ->
            Config = #{
                coded_pid => CodedPid, 
        ...
        {'DOWN', _Ref, process, FromPid, Reason} ->
            if Reason =/= normal ->
                ...
                if External_Id =/= false ->
                    logging:log(Id, "Drone synchronizer process for the synchronization with the drone ~p has crashed. A new one will be spawned.~n", [External_Id]),
